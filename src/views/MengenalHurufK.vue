<template>
    <div class="flex flex-col items-center bg-gradient-to-b from-green-200 to-blue-200 h-screen p-6">
        ...
        <div class="text-center mb-24 mt-10">
            <h1 class="text-9xl font-extrabold text-blue-700 drop-shadow-lg animate-bounce">K k</h1>
        </div>

        <!-- Images starting with the letter 'K' -->
        <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mb-16">
            <div v-for="(image, index) in images" :key="index" @click="openModal(image.name)"
                ...
                <div class="p-4 text-center text-xl font-bold text-gray-700">
                    <span class="text-blue-700">K</span>{{ image.alt.slice(1) }}
                </div>
            </div>
        </div>
        ...
        <div v-if="isModalOpen" @click.self="closeModal"
            ...
            <div class="bg-white rounded-lg shadow-xl p-8 max-w-md w-full relative grid place-content-center">
                ...
                <h2 class="text-4xl text-center font-bold mb-2">
                    <span class="text-blue-700 text-5xl">K</span>{{ currentImageAlt.slice(1) }}
                </h2>
...
// Letter K
import kasurImage from "../assets/mengenalHuruf/K/gambar_kasur.jpg";
import kucingImage from "../assets/mengenalHuruf/K/gambar_kucing.jpg";
import kueImage from "../assets/mengenalHuruf/K/gambar_kue.jpg";
import kupukupuImage from "../assets/mengenalHuruf/K/gambar_kupukupu.jpg";

import audioKasur from "../assets/mengenalHuruf/K/audio_kasur.mp3";
import audioKucing from "../assets/mengenalHuruf/K/audio_kucing.mp3";
import audioKue from "../assets/mengenalHuruf/K/audio_kue.mp3";
import audioKupukupu from "../assets/mengenalHuruf/K/audio_kupukupu.mp3";
import audioHurufK from "../assets/mengenalHuruf/K/audioHuruf_k.mp3";
...
const images = [
    { name: "kasur", src: kasurImage, alt: "Kasur", audio: new Audio(audioKasur) },
    { name: "kucing", src: kucingImage, alt: "Kucing", audio: new Audio(audioKucing) },
    { name: "kue", src: kueImage, alt: "Kue", audio: new Audio(audioKue) },
    { name: "kupukupu", src: kupukupuImage, alt: "Kupukupu", audio: new Audio(audioKupukupu) },
];

const replayAudio = new Audio(audioHurufK);
