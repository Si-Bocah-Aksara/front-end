<template>
    <div class="flex flex-col items-center bg-gradient-to-b from-green-200 to-blue-200 h-screen p-6">
        ...
        <div class="text-center mb-24 mt-10">
            <h1 class="text-9xl font-extrabold text-blue-700 drop-shadow-lg animate-bounce">T t</h1>
        </div>

        <!-- Images starting with the letter 'T' -->
        <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mb-16">
            <div v-for="(image, index) in images" :key="index" @click="openModal(image.name)"
                ...
                <div class="p-4 text-center text-xl font-bold text-gray-700">
                    <span class="text-blue-700">T</span>{{ image.alt.slice(1) }}
                </div>
            </div>
        </div>
        ...
        <div v-if="isModalOpen" @click.self="closeModal"
            ...
            <div class="bg-white rounded-lg shadow-xl p-8 max-w-md w-full relative grid place-content-center">
                ...
                <h2 class="text-4xl text-center font-bold mb-2">
                    <span class="text-blue-700 text-5xl">T</span>{{ currentImageAlt.slice(1) }}
                </h2>
...
// Huruf T
import tanganImage from "../assets/mengenalHuruf/T/gambar_tangan.jpg";
import tasImage from "../assets/mengenalHuruf/T/gambar_tas.jpg";
import telurImage from "../assets/mengenalHuruf/T/gambar_telur.jpg";
import tikusImage from "../assets/mengenalHuruf/T/gambar_tikus.jpg";

import audioTangan from "../assets/mengenalHuruf/T/audio_tangan.mp3";
import audioTas from "../assets/mengenalHuruf/T/audio_tas.mp3";
import audioTelur from "../assets/mengenalHuruf/T/audio_telur.mp3";
import audioTikus from "../assets/mengenalHuruf/T/audio_tikus.mp3";
import audioHurufT from "../assets/mengenalHuruf/T/audioHuruf_t.mp3";
...
const images = [
    { name: "tangan", src: tanganImage, alt: "Tangan", audio: new Audio(audioTangan) },
    { name: "tas", src: tasImage, alt: "Tas", audio: new Audio(audioTas) },
    { name: "telur", src: telurImage, alt: "Telur", audio: new Audio(audioTelur) },
    { name: "tikus", src: tikusImage, alt: "Tikus", audio: new Audio(audioTikus) },
];

const replayAudio = new Audio(audioHurufT);
