<template>
    <div class="flex flex-col items-center bg-gradient-to-b from-green-200 to-blue-200 h-screen p-6">
        ...
        <div class="text-center mb-24 mt-10">
            <h1 class="text-9xl font-extrabold text-blue-700 drop-shadow-lg animate-bounce">W w</h1>
        </div>

        <!-- Images starting with the letter 'W' -->
        <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mb-16">
            <div v-for="(image, index) in images" :key="index" @click="openModal(image.name)"
                ...
                <div class="p-4 text-center text-xl font-bold text-gray-700">
                    <span class="text-blue-700">W</span>{{ image.alt.slice(1) }}
                </div>
            </div>
        </div>
        ...
        <div v-if="isModalOpen" @click.self="closeModal"
            ...
            <div class="bg-white rounded-lg shadow-xl p-8 max-w-md w-full relative grid place-content-center">
                ...
                <h2 class="text-4xl text-center font-bold mb-2">
                    <span class="text-blue-700 text-5xl">W</span>{{ currentImageAlt.slice(1) }}
                </h2>
...
// Huruf W
import wajanImage from "../assets/mengenalHuruf/W/gambar_wajan.jpg";
import wajahImage from "../assets/mengenalHuruf/W/gambar_wajah.jpg";
import wayangImage from "../assets/mengenalHuruf/W/gambar_wayang.jpg";
import wortelImage from "../assets/mengenalHuruf/W/gambar_wortel.jpg";

import audioWajan from "../assets/mengenalHuruf/W/audio_wajan.mp3";
import audioWajah from "../assets/mengenalHuruf/W/audio_wajah.mp3";
import audioWayang from "../assets/mengenalHuruf/W/audio_wayang.mp3";
import audioWortel from "../assets/mengenalHuruf/W/audio_wortel.mp3";
import audioHurufW from "../assets/mengenalHuruf/W/audioHuruf_w.mp3";
...
const images = [
    { name: "wajan", src: wajanImage, alt: "Wajan", audio: new Audio(audioWajan) },
    { name: "wajah", src: wajahImage, alt: "Wajah", audio: new Audio(audioWajah) },
    { name: "wayang", src: wayangImage, alt: "Wayang", audio: new Audio(audioWayang) },
    { name: "wortel", src: wortelImage, alt: "Wortel", audio: new Audio(audioWortel) },
];

const replayAudio = new Audio(audioHurufW);
