<template>
    <div class="flex flex-col items-center bg-gradient-to-b from-green-200 to-blue-200 h-screen p-6">
        ...
        <div class="text-center mb-24 mt-10">
            <h1 class="text-9xl font-extrabold text-blue-700 drop-shadow-lg animate-bounce">S s</h1>
        </div>

        <!-- Images starting with the letter 'S' -->
        <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mb-16">
            <div v-for="(image, index) in images" :key="index" @click="openModal(image.name)"
                ...
                <div class="p-4 text-center text-xl font-bold text-gray-700">
                    <span class="text-blue-700">S</span>{{ image.alt.slice(1) }}
                </div>
            </div>
        </div>
        ...
        <div v-if="isModalOpen" @click.self="closeModal"
            ...
            <div class="bg-white rounded-lg shadow-xl p-8 max-w-md w-full relative grid place-content-center">
                ...
                <h2 class="text-4xl text-center font-bold mb-2">
                    <span class="text-blue-700 text-5xl">S</span>{{ currentImageAlt.slice(1) }}
                </h2>
...
// Huruf S
import saljuImage from "../assets/mengenalHuruf/S/gambar_salju.jpg";
import sandalImage from "../assets/mengenalHuruf/S/gambar_sandal.jpg";
import sapiImage from "../assets/mengenalHuruf/S/gambar_sapi.jpg";
import sepedaImage from "../assets/mengenalHuruf/S/gambar_sepeda.jpg";

import audioSalju from "../assets/mengenalHuruf/S/audio_salju.mp3";
import audioSendal from "../assets/mengenalHuruf/S/audio_sendal.mp3";
import audioSapi from "../assets/mengenalHuruf/S/audio_sapi.mp3";
import audioSepeda from "../assets/mengenalHuruf/S/audio_sepeda.mp3";
import audioHurufS from "../assets/mengenalHuruf/S/audioHuruf_s.mp3";
...
const images = [
    { name: "salju", src: saljuImage, alt: "Salju", audio: new Audio(audioSalju) },
    { name: "sandal", src: sandalImage, alt: "Sandal", audio: new Audio(audioSendal) },
    { name: "sapi", src: sapiImage, alt: "Sapi", audio: new Audio(audioSapi) },
    { name: "sepeda", src: sepedaImage, alt: "Sepeda", audio: new Audio(audioSepeda) },
];

const replayAudio = new Audio(audioHurufS);
